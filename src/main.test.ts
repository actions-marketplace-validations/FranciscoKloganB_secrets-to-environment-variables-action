...
jest
	.mocked(core.warning)
	.mockImplementation((s) => console.log(`WARNING: ${s}`))
...
describe('secrets-to-env-action', () => {
	...
	beforeEach(() => {
		...
		jest.mocked(core.exportVariable).mockImplementation((key, value) => {
	...
	it('adds a prefix', async () => {
		...
		const actual = Object.keys(newSecrets).every((value) =>
	...
	it('removes a prefix', async () => {
		...
		const actual = Object.keys(newSecrets).some((value) =>
			value.startsWith(prefix),
		)
